...
router.post('/register',async (req, res) => {
    try {
      const user = await Nurse.findOne({email:req.body.email});
      if (user) {
        res.status(400).json({respnse:"Nurse Exists"});
      }
      else{
...
router.post('/login', async (req, res) => {
      try{
        ...
        const user = await Nurse.findOne({ email:email });
...
router.get('/getdetails', async (req, res) => {
  try{
    const nurse=await Nurse.findById(req.user._id).populate({
      path: 'patients',
